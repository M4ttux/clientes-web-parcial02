...
import { getUserDisplayName } from '../services/user-profile';
...
export default {
  ...
  methods: {
    ...
    async loadUserProfile() {
      ...
      const data = await getUserDisplayName(this.user.id);
      if (data) {
    ...
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
      ...
      <div :class="['w-full md:block md:w-auto', menuOpen ? 'block' : 'hidden']" id="navbar-default">
        <ul
          ...
          <!-- Link a Home -->
          <li>
            <router-link to="/" class="block py-2 px-3 rounded-lg hover:text-blue-400 md:p-0"
          ...
          <template v-if="!user.id">
            <li>
              <router-link to="/ingresar" class="block py-2 px-3 hover:text-blue-400 md:p-0">Iniciar
                Sesión</router-link>
            </li>
            <li>
          ...
          <template v-else>
            <li>
              <router-link to="/chat-global" class="block py-2 px-3 hover:text-blue-400 md:p-0">Chat
                Global</router-link>
            </li>
            <li>
